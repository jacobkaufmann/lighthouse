...
pub fn scrape_for_metrics<U: SlotClock>(clock: &U) {
    ...
    let present_epoch = present_slot.epoch(clock.slots_per_epoch());

    set_gauge(&PRESENT_SLOT, present_slot.as_u64() as i64);
    set_gauge(&PRESENT_EPOCH, present_epoch.as_u64() as i64);
    set_gauge(&SLOTS_PER_EPOCH, clock.slots_per_epoch() as i64);
    set_gauge(
        &SECONDS_PER_SLOT,
        clock.slot_duration(present_epoch).as_secs() as i64,
    );
}
